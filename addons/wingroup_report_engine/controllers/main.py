...
class APIAll(http.Controller):
    ...
    @http.route('/api/create/pdf-by-xml', auth='public', csrf=False, type='json')
    def api_create_pdf_by_xml(self, **kwargs):
        try:
            Authorization = request.httprequest.headers.environ.get("HTTP_AUTHORIZATION")
            user = request.env['res.users'].sudo().get_api_rest_user(Authorization.replace('Bearer ', ''))
            if not user:
            ...
            report_data = {key: request.jsonrequest.get(key) for key in request.jsonrequest}
            new_url = request.env['wg.report.template.odt'].with_user(user).action_print_by_xml(data=report_data)
            return {
                'status': 'Success',
                'message': new_url,
            }
    ...
    @http.route('/api/create/json-by-xml', auth='public', csrf=False, type='json')
    def api_create_json_by_xml(self, **kwargs):
        try:
            Authorization = request.httprequest.headers.environ.get("HTTP_AUTHORIZATION")
            user = request.env['res.users'].sudo().get_api_rest_user(Authorization.replace('Bearer ', ''))
            if not user:
            ...
            report_data = {key: request.jsonrequest.get(key) for key in request.jsonrequest}
            data_json = request.env['wg.report.template.odt'].with_user(user).tvan_xml_to_json(data=report_data)
            return {
                'result': data_json,
            }
        except Exception as e:
